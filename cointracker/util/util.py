...
def findSaleMatch(order, poolList, invMethod):
        ...
        for index, _ in poolList.iterrows():
            ...
            if poolList.loc[index, 'Asset'] == sellAsset:
                ...
                if poolList.loc[index, 'Active']:
                    ...
                    # take away the Abs()
                    perc = abs((sellAmount - poolList.loc[index, 'Amount'])
    ...
    try:
        ...
        # raise ValueError(errorString)
        # ctypes.windll.user32.MessageBoxW(0, errorString, "Error", 0)
        print('----------Error: No valid purchase pool match for', sellAsset, 'found ----------')
        print('Attempted Sell Amount: ', sellAmount)
        print('Available ', sellAsset, ' Supply: ', poolList[(poolList["Asset"] == sellAsset) &
              poolList["Active"]]['Amount'].sum())
        print('----------------------------------------------------------------')
        return 0, 0, 0
...
def getIDIndex(pools, ID):
    ...
    if len(poolSet) == 0:
        ...
        print('Error: ', len(poolSet), ' instances of ID found. Returning the first index found')
...
def getIDIndexOld(pools, ID):
    ...
    if idIndex == -1:
        print('Error: Cannot find ID index for ID ', ID)
    if iterations > 1:
        print('Error: ', iterations, ' instances of ID found. Returning only the last index found')
...
def buyAsset(order, purchasePools, salePools, invMethod):
    ...
    if order.loc[0, 'Price'] == 0:
        ...
        purchaseCostBasis = sellAmount*assetSoldSpot + \
            purchaseFeeUSD  # doesn't work for free items with cost=0
    ...
    if (assetReceived != 'USD') & (emptyOrder == False):
        ...
        purchasePools = purchasePools.append(pPool, ignore_index=True)
        ...
        print('orderType should be BUY: ', orderType)

        purchasePools, salePools = washPoolMatches(pID, 'BUY', invMethod, purchasePools, salePools)
...
def sellAsset(order, purchasePools, salePools, invMethod):
    ...
    if assetSold != 'USD':
        ...
        costBasis = purchasePools.loc[pIndex, 'Modified Cost Basis']  # needs to used the modified cost basis
        ...
        # and generate the sale pool
            ...
            remainingPoolAmount = pPoolAmount-sellAmount  # purchasePool amount - salePool amount
            percentUsed = \
                sellAmount/pPoolAmount  # calculate before updating the original purchasePool

            # create new pool copying the existing purchase pool
            newPurchasePool = purchasePools.loc[pIndex:pIndex].copy()
            pID = makeID(purchasePools)  # update pID, which is used later for the pool that is being closed
            newPurchasePool.loc[pIndex, 'Pool ID'] = pID  # NOTE: pIndex has not changed yet

            # update the new pool
            newPurchasePool.loc[pIndex, 'Amount'] = sellAmount  # should equal newPurchasePool.loc[pIndex,'Amount']*percentUsed
            newPurchasePool.loc[pIndex, 'Fee'] = newPurchasePool.loc[pIndex, 'Fee']*percentUsed
            newPurchasePool.loc[pIndex, 'Fee USD'] = round(newPurchasePool.loc[pIndex, 'Fee USD']*percentUsed, USDdecimals)
            newPurchasePool.loc[pIndex, 'Cost Basis'] = round(costBasis*percentUsed, USDdecimals)
            # modified cost basis too?
            newPurchasePool.loc[pIndex, 'Modified Cost Basis'] = round(newPurchasePool.loc[pIndex, 'Modified Cost Basis']*percentUsed, USDdecimals)
            newPurchasePool.loc[pIndex, 'Active'] = False  # close the new pool

            # update old pool
            purchasePools.loc[pIndex, 'Amount'] = \
                purchasePools.loc[pIndex, 'Amount']*(1-percentUsed) # should equal newPurchasePool.loc[pIndex,'Amount']*percentUsed
            purchasePools.loc[pIndex, 'Fee'] = purchasePools.loc[pIndex, 'Fee']*(1-percentUsed)
            purchasePools.loc[pIndex, 'Fee USD'] = \
                round(purchasePools.loc[pIndex, 'Fee USD']*(1-percentUsed), USDdecimals)
            purchasePools.loc[pIndex, 'Cost Basis'] = \
                round(purchasePools.loc[pIndex, 'Cost Basis']*(1-percentUsed), USDdecimals)
            # modified cost basis too?
            purchasePools.loc[pIndex, 'Modified Cost Basis'] = \
                round(purchasePools.loc[pIndex, 'Modified Cost Basis']*(1-percentUsed), USDdecimals)
            ...
            if newPurchasePool.loc[pIndex, 'Modified Cost Basis'] > poolLimitUSD:
                ...
                purchasePools = purchasePools.append(newPurchasePool, ignore_index=True)
            else:
                purchasePools.loc[pIndex, 'Active'] = True  # the old pool becomes inactive
            ...
            costBasis = costBasis*percentUsed  # this doesn't use the modified cost basis
        ...
        # create a new sell order with the remaining amount
            ...
            remainingSaleAmount = sellAmount-pPoolAmount  # salePool amount - purchasePool amount
            ...
            remainingOrder.loc[0, 'Fee'] = 0  # let the first salePool contain the entirety of the
            # fees...fewer adjustments
            remainingOrder.loc[0, 'Total'] = remainingOrder.loc[0, 'Total']*(1-percentUsed)
        ...
        sPool = {'Pool ID': sID, 'Asset Sold': assetSold, 'Asset Received': assetReceived,
                 'Purchase Date': purchaseDate, 'Sale Date': saleDate, 'Amount': sellAmount,
                 'Sale Price': salePrice, 'Asset Sold Spot Price': assetSoldSpot,
                 'Asset Received Spot Price': assetReceivedSpot, 'Fee': saleFee,
                 'Fee Coin': saleFeeCoin, 'Fee Coin Spot Price': saleFeeCoinSpot,
                 'Fee USD': round(saleFeeUSD, USDdecimals), 'Purchase Pool ID': pID,
                 'Holding Period': holdingPeriod, 'Long': long,
                 'Proceeds': round(proceeds, USDdecimals),
                 'Cost Basis': round(costBasis, USDdecimals), 'Wash Pool ID': washID,
...
def getEmptySalePool():
    sCols = ['Pool ID', 'Asset Sold', 'Asset Received', 'Purchase Date', 'Sale Date', 'Amount',
             'Sale Price', 'Asset Sold Spot Price', 'Asset Received Spot Price',
    ...
    typeDict = {'Pool ID': 'int64', 'Asset Sold': 'object', 'Asset Received': 'object',
                'Purchase Date': 'datetime64[ns, UTC]', 'Sale Date': 'datetime64[ns, UTC]',
                'Amount': 'float64', 'Sale Price': 'float64', 'Asset Sold Spot Price': 'float64',
                'Asset Received Spot Price': 'float64', 'Fee': 'float64', 'Fee Coin': 'object',
                'Fee Coin Spot Price': 'float64', 'Fee USD': 'float64',
                'Purchase Pool ID': 'int64', 'Holding Period': 'timedelta64[ns]', 'Long': 'bool',
                'Proceeds': 'float64', 'Cost Basis': 'float64', 'Wash Pool ID': 'int64',
                'Disallowed Loss': 'float64', 'Net Gain': 'float64'}
...
def getEmptyPurchasePool():
    pCols = ['Pool ID', 'Active', 'Purchase Date', 'Asset', 'Asset Spot Price', 'Amount',
             'Fee', 'Fee Coin', 'Fee Coin Spot Price', 'Fee USD',
             'Initiates Wash', 'Holding Period Modifier', 'Cost Basis', 'Modified Cost Basis']
    typeDict = {'Pool ID': 'int64', 'Active': 'bool', 'Purchase Date': 'datetime64[ns, UTC]',
                'Asset': 'object', 'Asset Spot Price': 'float64', 'Amount': 'float64',
                'Fee': 'float64', 'Fee Coin': 'object', 'Fee Coin Spot Price': 'float64',
                'Fee USD': 'float64', 'Initiates Wash': 'bool',
                'Holding Period Modifier': 'timedelta64[ns]', 'Cost Basis': 'float64',
...
def getEmptyOrder():
    pCols = ['Date(UTC)', 'Market', 'Type', 'Price', 'Amount', 'Total',
             'Fee', 'Fee Coin', 'Market 1 USD Spot Price', 'Market 2 USD Spot Price',
             'Fee Coin USD Spot Price']
    typeDict = {'Date(UTC)': 'datetime64[ns, UTC]', 'Market': 'object', 'Type': 'object',
                'Price': 'float64', 'Amount': 'float64', 'Total': 'float64', 'Fee': 'float64',
                'Fee Coin': 'object', 'Market 1 USD Spot Price': 'float64',
                'Market 2 USD Spot Price': 'float64', 'Fee Coin USD Spot Price': 'float64'}
    purchasePool = pd.DataFrame(columns=pCols)
    purchasePool = purchasePool.astype(typeDict)
...
def washPoolMatches(ID, orderType, invMethod, purchasePools, salePools):
    ...
    cutoffDate = exchangeDate - window  # earliest date for which a previous purchase could be considered a wash
    ...
    if orderType == 'BUY':
        ...
        potentialPools = salePools[(salePools['Sale Date'] > cutoffDate) &
                                   (salePools['Asset Sold'] == asset) &
                                   (salePools['Wash Pool ID'] == 0) & (salePools['Net Gain'] < 0)]
        if invMethod == 'FIFO':
            potentialPools = potentialPools.sort_values(by=['Sale Date'], ascending=True)
        elif invMethod == 'LIFO':
            potentialPools = potentialPools.sort_values(by=['Sale Date'], ascending=False)
        else:
        ...
        if len(potentialPools) > 0:  # if there are, in fact, any wash sales identified
            ...
            if amount > sPoolAmount:
                ...
                washBuyPool = purchasePools.loc[[nbIndex]].copy()  # copy actually isn't necessary here
                washBuyPool.loc[nbIndex, 'Amount'] = washBuyPool.loc[nbIndex, 'Amount']*washPercent
                washBuyPool.loc[nbIndex, 'Fee'] = washBuyPool.loc[nbIndex, 'Fee']*washPercent
                washBuyPool.loc[nbIndex, 'Fee USD'] = round(washBuyPool.loc[nbIndex, 'Fee USD']*washPercent, USDdecimals)
                ...
                washBuyPool.loc[nbIndex, 'Cost Basis'] = round(washBuyPool.loc[nbIndex,'Cost Basis']*washPercent, USDdecimals)
                # Cost Basis include fees already, correct?
                # Net Gain is negative here as it is a loss
                washBuyPool.loc[nbIndex, 'Modified Cost Basis'] = round(washBuyPool.loc[nbIndex, 'Cost Basis']-salePools.loc[sIndex, 'Net Gain'], USDdecimals)
                washBuyID = makeID(purchasePools)
                washBuyPool.loc[nbIndex, 'Pool ID'] = washBuyID

                # HUH? WHY SHOULD IT ALREADY BE CLOSED?
                print('purchasePool should already be closed: ', washBuyPool.loc[nbIndex, 'Active']) #why should it already be closed?
                ...
                if washBuyPool.loc[nbIndex, 'Modified Cost Basis'] > poolLimitUSD:
                    ...
                    purchasePools.loc[nbIndex, 'Amount'] = purchasePools.loc[nbIndex, 'Amount']*(1-washPercent)
                    purchasePools.loc[nbIndex, 'Fee'] = purchasePools.loc[nbIndex, 'Fee']*(1-washPercent)
                    purchasePools.loc[nbIndex, 'Fee USD'] = round(purchasePools.loc[nbIndex, 'Fee USD']*(1-washPercent), USDdecimals)
                    purchasePools.loc[nbIndex, 'Cost Basis'] = round(purchasePools.loc[nbIndex, 'Cost Basis']*(1-washPercent), USDdecimals)
                    purchasePools.loc[nbIndex, 'Modified Cost Basis'] = round(purchasePools.loc[nbIndex, 'Modified Cost Basis']*(1-washPercent), USDdecimals)

                    remainingBuyID = purchasePools.loc[nbIndex, 'Pool ID']

                    # append the washBUY to the purchasePools list
                    purchasePools = purchasePools.append(washBuyPool, ignore_index=True)

                    # absorb the losses by updating the washSalePool
                    salePools.loc[sIndex, 'Wash Pool ID'] = washBuyID

                    # Cost Basis include fees already, correct?
                    salePools.loc[sIndex, 'Disallowed Loss'] = abs(round(salePools.loc[sIndex, 'Net Gain'], USDdecimals)) #disallowed loss is the previous net gain

                    # Net Gain is negative here as it is a loss
                    salePools.loc[sIndex, 'Net Gain'] = 0  # net gain is added to the cost basis of the new BUY pool
                    ...
                    purchasePools, salePools = washPoolMatches(remainingBuyID, 'BUY', invMethod, purchasePools, salePools)
                # if not treat it as if the amounts are equal
                    ...
                    purchasePools.loc[nbIndex,'Modified Cost Basis'] =  round(purchasePools.loc[nbIndex, 'Cost Basis']-salePools.loc[sIndex, 'Net Gain'], USDdecimals)

                    # absorb the losses by updating the washSalePool
                    salePools.loc[sIndex, 'Wash Pool ID'] = ID  # ID is the wash purchasePool ID in this case with 'BUY'
                    # Cost Basis include fees already, correct?
                    salePools.loc[sIndex, 'Disallowed Loss'] = abs(round(salePools.loc[sIndex, 'Net Gain'], USDdecimals)) #disallowed loss is the previous net gain
                ...
                washSalePool = salePools.loc[[sIndex]].copy()  # copy actually isn't necessary here
                washSalePool.loc[sIndex, 'Amount'] = washSalePool.loc[sIndex, 'Amount']*washPercent
                washSalePool.loc[sIndex, 'Fee'] = washSalePool.loc[sIndex, 'Fee']*washPercent
                washSalePool.loc[sIndex, 'Fee USD'] = round(washSalePool.loc[sIndex, 'Fee USD']*washPercent,USDdecimals)
                washSalePool.loc[sIndex, 'Cost Basis'] = round(washSalePool.loc[sIndex, 'Cost Basis']*washPercent, USDdecimals)
                washSalePool.loc[sIndex, 'Proceeds'] = round(washSalePool.loc[sIndex, 'Proceeds']*washPercent, USDdecimals)
                washLoss = round(washSalePool.loc[sIndex, 'Net Gain']*washPercent, USDdecimals)
                washSaleID = makeID(salePools)
                washSalePool.loc[sIndex, 'Pool ID'] = washSaleID
                washSalePool.loc[sIndex, 'Wash Pool ID'] = ID  # ID is the wash purchasePool ID in this case with 'BUY'

                # Cost Basis include fees already, correct?
                # Net Gain is negative here as it is a loss
                washSalePool.loc[sIndex, 'Disallowed Loss'] = abs(washLoss)
                washSalePool.loc[sIndex, 'Net Gain'] = 0  # net gain is added to the cost basis of the new BUY pool

                # append the washSalePool to the salePools list
                salePools = salePools.append(washSalePool, ignore_index=True)                
                ...
                # Cost Basis include fees already, correct?
                # Net Gain is negative here as it is a loss...NEEDS TO BE DONE BEFORE UPDATING THE washSalePool (salePool as sIndex)
                purchasePools.loc[nbIndex, 'Modified Cost Basis'] = round(purchasePools.loc[nbIndex, 'Cost Basis']-washLoss)

                # remaining salePool
                salePools.loc[sIndex, 'Amount'] = salePools.loc[sIndex, 'Amount']*(1-washPercent)
                salePools.loc[sIndex, 'Fee'] = salePools.loc[sIndex, 'Fee']*(1-washPercent)
                salePools.loc[sIndex, 'Fee USD'] = round(salePools.loc[sIndex, 'Fee USD']*(1-washPercent), USDdecimals)
                salePools.loc[sIndex, 'Cost Basis'] = round(salePools.loc[sIndex, 'Cost Basis']*(1-washPercent), USDdecimals)
                salePools.loc[sIndex, 'Proceeds'] = round(salePools.loc[sIndex, 'Proceeds']*(1-washPercent), USDdecimals)
                salePools.loc[sIndex, 'Net Gain'] = salePools.loc[sIndex, 'Proceeds']-salePools.loc[sIndex, 'Cost Basis']
        ...
        if invMethod == 'LIFO':
            ...
            potentialPools = purchasePools[(purchasePools['Purchase Date']>cutoffDate) & (purchasePools['Asset']==asset)]
            potentialPools = potentialPools.sort_values(by=['Purchase Date'], ascending = True)
        else:
            print('Error: Invalid invMethod')
    else:
        print('Error: unidentifiable transaction method (BUY or SELL).')
...
def prettyPrintPools(poolDB,filename):
    ...
    Cols = ['Asset Sold', 'Purchase Date','Sale Date', 'Amount', 'Spot Price (USD)', 'Fee (USD)', 'Holding Period (days)', 
        'Short/Long', 'Proceeds (USD)', 'Cost Basis (USD)', 'Wash Sale', 'Disallowed Loss (USD)', 'Net Gain (USD)']
    ...
    pool['Spot Price (USD)'] = poolDB['Sale Price']
    ...
    pool.to_csv(filename, index=False, encoding='utf-8')
    return
